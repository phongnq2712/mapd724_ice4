...
class CollisionManager
{
    ...
    public static func SquareRadiusCheck(scence: SKScene, object1: GameObject, object2: GameObject) -> Void
    {
        ...
        if(SquaredDistance(point1: P1, point2: P2) < Radii * Radii)
        {
            ...
            if(!object2.isCollding!)
            {
                ...
                case "island":
                    ...
                    scence.run(SKAction.playSoundFileNamed("yay", waitForCompletion: false))
                    print("Collided with Island")
                    break
                case "cloud":
                    ScoreManager.Lives -= 1
                    gameViewController?.updateLivesLabel()
                    scence.run(SKAction.playSoundFileNamed("thunder", waitForCompletion: false))
